defmodule ACS.Session do
  ...
  @doc """
    ...
    Ths supervisors starts the sessions from start_child

  """
  def start_link(session_module,
    ...
    Logger.debug(
      "ACS.Session start_link(#{inspect(session_id)},#{inspect(device_id)}) with function"
    )

    Gauge.inc(name: :acs_ex_nof_sessions, labels: [device_id.product_class])
    GenServer.start_link(__MODULE__, [session_module, session_id, device_id, message, fun])
  end
  ...
  def start_link(session_module,
    ...
    Gauge.inc(name: :acs_ex_nof_sessions, labels: [device_id.product_class])
    GenServer.start_link(__MODULE__, [session_module, session_id, device_id, message, nil])
  end
  ...
  def process_message(session_id, message) do
    try do
      timeout =
        case Application.fetch_env(:acs_ex, :session_timeout) do
          {:ok, to} -> to
          :error -> 30000
        end
      ...
      :exit, reason ->
        case reason do
          # Generate fault response? Or maybee just end the session by returning ""
          {:timeout, _} ->
            ...
            CWMP.Protocol.Generator.generate(
              ...
              %CWMP.Protocol.Messages.Fault{
                faultcode: "Server",
                faultstring: "CWMP fault",
                detail: %CWMP.Protocol.Messages.FaultStruct{
                  code: "8002",
  ...
  def script_command(device_id, command) do
    ...
    try do
      timeout =
        case Application.fetch_env(:acs_ex, :script_timeout) do
          {:ok, to} -> to
          :error -> 2000
        end
      ...
      :exit, reason ->
        case reason do
          ...
          {what, ever} ->
            Logger.debug("Whatever occured #{inspect(what)}, #{inspect(ever)}")
            {what, ever}
        end
    end
  end
  ...
  def init([script_module, session_id, device_id, message, fun]) do
    ...
    gspid = self()
    # TODO: Should be "first inform encountered", not just hd
    sspid =
      spawn_link(__MODULE__, :session_prestart, [
        gspid,
        script_module,
        device_id,
        hd(message.entries),
      ...
      :ok ->
        ...
        {:ok,
         %{
           ...
           script_element: nil,
           plug_element: nil,
           unmatched_incomming_list: [],
           sspid: sspid,
           cwmp_version: message.cwmp_version
         }}

      _ ->
        {:stop, "Could not take over session"}
    end
  end
  ...
  def handle_info({:EXIT, _pid, _reason}, state) do
    ...
    case state.plug_element do
      nil ->
        Logger.debug(
          "Session script exited, and we have no waiting plug, leave the plug some time to end session"
        )

      pe ->
        # Waiting plug, we have to tell it to stop by sending {204,""}
        Logger.debug(
          "Waiting plug when SS ends, just tell it to stop, which in turn will kill me (the session)"
        )
  ...
  def handle_call({:script_command, [command]}, from, state) do
    ...
    case state.plug_element do
      %{message: _msg, from: plug_from, state: :waiting} ->
        ...
        Logger.debug(
          "Session Script discovered a waiting plug. Sending scripted command at once!"
        )
          ...
          {:ok, {id, req}} ->
            ...
            {:noreply,
             %{
               state
               | plug_element: nil,
                 script_element: %{command: command, from: from, state: :sent, id: id}
             }}
      ...
      _ ->
        Logger.debug(
          "No known plug_element, meaning no plug is waiting, so store script command in state"
        )
        ...
        case state.script_element do
          nil ->
            {:noreply,
             %{state | script_element: %{command: command, from: from, state: :unhandled}}}
  ...
  @doc """
  :verify_remotehost - verifies the remote_host by comparing it to the one in the state.device_id

  :process_message - Processes a message from the plug. message is the CWMP.Protocol version of
  ...
  def handle_call({:process_message, [session_id, message]}, from, state) do
    ...
    # now.
    {plug, script, unmatched, sspid} =
      case length(Map.keys(message)) do
        0 ->
          ...
          Logger.debug(
            "Empty message discovered: sspid: #{inspect(state.sspid)}, script_element: #{inspect(state.script_element)}"
          )
          ...
          case state.script_element do
              ...
              case state.sspid do
                ...
                sspid ->
                  ...
                  if Process.alive?(sspid) do
                    ...
                    Logger.debug(
                      "Script system is not actually alive, it only seems so. Missed an :exit?"
                    )
              ...
              case gen_request(command.method, command.args, "script", state.cwmp_version) do
                {:ok, {id, req}} ->
                  {{200, req}, %{command: command, from: script_from, state: :sent, id: id},
                   state.unmatched_incomming_list, state.sspid}
            ...
            _ ->
              Logger.debug(
                "Cant identify script_element, clearing and discontinuing session: #{inspect(state.script_element)}"
              )
        ...
        3 ->
          Logger.debug(
            "CWMP message discovered: script_element: #{inspect(state.script_element)}"
          )
          ...
          case has_inform?(message.entries) do
            true ->
              ...
              id =
                if !is_nil(message.header) && Map.has_key?(message.header, :id) do
                  message.header.id
                else
                  0
                end
              ...
              {{200,
                CWMP.Protocol.Generator.generate!(
                  ...
                  %CWMP.Protocol.Messages.InformResponse{max_envelopes: 1},
                  message.cwmp_version
                )}, state.script_element, [], state.sspid}
            ...
            false ->
              case state.script_element do
                ...
                %{command: :unscripted, from: _from, state: :unhandled} ->
                  Logger.debug(
                    "We have a script wanting the unmatched list - we should still reply to this though"
                  )
                ...
                %{command: _command, from: from, state: :sent, id: generated_header_id} ->
                  ...
                  if message.header.id == generated_header_id do
                    ...
                    Logger.debug(
                      "Incomming message is unmatched to script - we should reply somehow?"
                    )
    ...
    Logger.debug(
      "process_message returning with #{inspect(plug)}, #{inspect(script)}, #{inspect(unmatched)} #{inspect(sspid)}"
    )
    ...
    case plug do
      :noreply ->
        {:noreply,
         %{
           state
           | plug_element: %{message: message, from: from, state: :waiting},
             unmatched_incomming_list: state.unmatched_incomming_list ++ unmatched,
             sspid: sspid
         }}
  ...
  def session_prestart(gspid, script_module, device_id, message, sessionid, fun) do
    ...
    case fun do
      ...
      f when is_function(f) ->
        apply(fun, [gspid, device_id, message])

      _ ->
        Logger.error("Can not figure out how to call the session_start function")
    end
  end
    ...
    case message_type(entry) do
      ...
      {:acs, CWMP.Protocol.Messages.GetRPCMethods} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.GetRPCMethodsResponse{
              methods: [
                "GetRPCMethods",
                "Inform",
                "TransferComplete",
      ...
      {:acs, CWMP.Protocol.Messages.TransferComplete} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.TransferCompleteResponse{},
            message.cwmp_version
          )}, [message]}
      ...
      {:acs, CWMP.Protocol.Messages.AutonomousTransferComplete} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.AutonomousTransferCompleteResponse{},
            message.cwmp_version
          )}, [message]}
      ...
      {:acs, CWMP.Protocol.Messages.Kicked} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.KickedResponse{next_url: entry.next},
            message.cwmp_version
          )}, [message]}
      ...
      {:acs, CWMP.Protocol.Messages.RequestDownload} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.RequestDownloadResponse{},
            message.cwmp_version
          )}, [message]}
      ...
      {:acs, CWMP.Protocol.Messages.DUStateChangeComplete} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.DUStateChangeCompleteResponse{},
            message.cwmp_version
          )}, [message]}
      ...
      {:acs, CWMP.Protocol.Messages.AutonomousDUStateChangeComplete} ->
        {{200,
          CWMP.Protocol.Generator.generate!(
            %CWMP.Protocol.Messages.Header{id: id},
            %CWMP.Protocol.Messages.AutonomousDUStateChangeCompleteResponse{},
            message.cwmp_version
          )}, [message]}
  ...
  defp gen_request(method, args, _source, cwmp_version) do
    ...
    case validateArgs(method, args) do
        ...
        message =
          case method do
            "GetRPCMethods" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetRPCMethods{},
                cwmp_version
              )
		...
		case args do
		  ...
		  m when is_map(m) ->
            ...
            "GetParameterValues" ->
              params =
                for a <- args,
                    do: %CWMP.Protocol.Messages.GetParameterValuesStruct{name: a, type: "string"}

              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetParameterValues{parameters: params},
                cwmp_version
              )

            "GetParameterNames" ->
              params = %CWMP.Protocol.Messages.GetParameterNames{
            ...
            "SetParameterAttributes" ->
              params =
                for a <- args,
                    do: %CWMP.Protocol.Messages.SetParameterAttributesStruct{
                      name: a.name,
              ...
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.SetParameterAttributes{parameters: params},
                cwmp_version
              )
            ...
            "GetParameterAttributes" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetParameterAttributes{parameters: args},
                cwmp_version
              )
            ...
            "AddObject" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.AddObject, args),
                cwmp_version
              )
            ...
            "DeleteObject" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.DeleteObject, args),
                cwmp_version
              )
            ...
            "Download" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.Download, args),
                cwmp_version
              )
            ...
            "GetQueuedTransfers" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetQueuedTransfers{},
                cwmp_version
              )
            ...
            "ScheduleInform" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.ScheduleInform, args),
                cwmp_version
              )

            "SetVouchers" ->
              voucherlist =
                for xmlsig <- args, do: struct(CWMP.Protocol.Messages.XMLSignatureStruct, xmlsig)

              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.SetVouchers{voucherlist: voucherlist},
                cwmp_version
              )
            ...
            "GetOptions" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetOptions{option_name: args},
                cwmp_version
              )
            ...
            "Upload" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.Upload, args),
                cwmp_version
              )
            ...
            "FactoryReset" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.FactoryReset{},
                cwmp_version
              )
            ...
            "GetAllQueuedTransfers" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.GetAllQueuedTransfers{},
                cwmp_version
              )
            ...
            "ScheduleDownload" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.ScheduleDownload, args),
                cwmp_version
              )
            ...
            "CancelTransfer" ->
              CWMP.Protocol.Generator.generate!(
                header,
                %CWMP.Protocol.Messages.CancelTransfer{commandkey: args},
                cwmp_version
              )
            ...
            "ChangeDUState" ->
              CWMP.Protocol.Generator.generate!(
                header,
                struct(CWMP.Protocol.Messages.ChangeDUState, args),
                cwmp_version
              )
      ...
      false ->
        {:error, "arguments for request #{method} do not validate"}
    end
  end
	...
	parameter_list && Enum.all?(parameter_list, fn a ->
      ...
      "SetParameterAttributes" ->
        ...
        case args do
          l when is_list(l) and length(l) > 0 ->
            Enum.all?(args, fn a ->
              Map.has_key?(a, :name) and Map.has_key?(a, :notification_change) and
                Map.has_key?(a, :notification) and Map.has_key?(a, :accesslist_change) and
                Map.has_key?(a, :accesslist) and is_list(a.accesslist)
            end)
      ...
      "Reboot" ->
        # takes no params, always true
        true

      "Download" ->
        is_map(args) and Map.has_key?(args, :url) and Map.has_key?(args, :filesize) and
          Map.has_key?(args, :filetype)
      ...
      "SetVouchers" ->
        ...
        if is_list(args) do
          Enum.all?(args, fn a ->
            if Map.has_key?(a, :signature_value) and Map.has_key?(a, :key_info) and
                 Map.has_key?(a, :options) do
              Logger.debug("step1")

              if is_list(a.options) and length(a.options) > 0 and
                   Map.has_key?(a.key_info, :key_value) and Map.has_key?(a.key_info, :x509_data) do
                Logger.debug("step2")

                if Map.has_key?(a.key_info.key_value, :dsa_p) and
                     Map.has_key?(a.key_info.key_value, :dsa_q) and
                     Map.has_key?(a.key_info.key_value, :dsa_g) and
                     Map.has_key?(a.key_info.key_value, :dsa_y) do
                  Logger.debug("step3")

                  if Map.has_key?(a.key_info.x509_data, :issuer_serial) and
                       Map.has_key?(a.key_info.x509_data, :subject_name) and
                       Map.has_key?(a.key_info.x509_data, :certificates) and
                       is_list(a.key_info.x509_data.certificates) and
                       Map.has_key?(a.key_info.x509_data.issuer_serial, :issuer_name) and
                       Map.has_key?(a.key_info.x509_data.issuer_serial, :serial_number) do
                    Logger.debug("step4")
                    # check all the options
                    matching =
                      Enum.all?(a.options, fn o ->
                        Map.has_key?(o, :v_serial_num) and Map.has_key?(o, :deviceid) and
                          Map.has_key?(o, :option_ident) and Map.has_key?(o, :option_desc) and
                          Map.has_key?(o, :start_date) and Map.has_key?(o, :duration) and
                          Map.has_key?(o, :duration_units) and Map.has_key?(o, :mode) and
                          Map.has_key?(o, :sha1_digest) and
                          Map.has_key?(o.deviceid, :manufacturer) and
                          Map.has_key?(o.deviceid, :oui) and
                          Map.has_key?(o.deviceid, :product_class) and
                          Map.has_key?(o.deviceid, :serial_number)
                      end)
      ...
      "Upload" ->
        ...
        is_map(args) and Map.has_key?(args, :commandkey) and Map.has_key?(args, :url) and
          Map.has_key?(args, :filetype)
      ...
      "ScheduleDownload" ->
        if is_map(args) and Map.has_key?(args, :url) and Map.has_key?(args, :filesize) and
             Map.has_key?(args, :filetype) and Map.has_key?(args, :timewindowlist) and
             is_list(args.timewindowlist) and length(args.timewindowlist) > 0 do
          # Check that all elements of the timelist list conform
          Enum.all?(args.timewindowlist, fn tw ->
            is_map(tw) and Map.has_key?(tw, :window_start) and Map.has_key?(tw, :window_end) and
              Map.has_key?(tw, :window_mode) and Map.has_key?(tw, :max_retries)
          end)
        else
          false
        end
      ...
      "ChangeDUState" ->
        if is_map(args) and Map.has_key?(args, :commandkey) and Map.has_key?(args, :operations) and
             is_list(args.operations) and length(args.operations) > 0 do
            ...
            if is_map(o) do
              case o do
                %CWMP.Protocol.Messages.InstallOpStruct{} ->
                  Map.has_key?(o, :url) and Map.has_key?(o, :uuid) and Map.has_key?(o, :username) and
                    Map.has_key?(o, :password) and Map.has_key?(o, :execution_env_ref)

                %CWMP.Protocol.Messages.UpdateOpStruct{} ->
                  Map.has_key?(o, :url) and Map.has_key?(o, :uuid) and Map.has_key?(o, :username) and
                    Map.has_key?(o, :password) and Map.has_key?(o, :version)

                %CWMP.Protocol.Messages.UninstallOpStruct{} ->
                  Map.has_key?(o, :url) and Map.has_key?(o, :uuid) and
                    Map.has_key?(o, :execution_env_ref)
              end
            else
              false
            end
